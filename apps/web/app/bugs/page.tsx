              ...
              <div className="max-h-[70vh] overflow-y-auto">
                <table className="w-full table-fixed text-base">
                  ...
                  <tbody className="divide-y">
                    {bugs.map((b) => (
                      <tr key={b.id} className="hover:bg-[hsl(var(--muted))]/30">
                        <td className="px-4 py-3 align-top">
                          <StatusBadge tone={severityTone(b.severity)}>{b.severity}</StatusBadge>
                        </td>
                        <td className="px-4 py-3 align-top">
                          <Link href={`/bugs/${b.id}`} className="block truncate text-[15px] font-semibold hover:underline">
                            {b.title}
                          </Link>
                          <div className="mt-0.5 truncate text-sm text-[hsl(var(--muted-foreground))]">
                            {b.description}
                          </div>
                        </td>
